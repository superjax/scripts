...
trim = data.estimator.ekf.state.time < time(end) & data.estimator.ekf.state.time > time(1);
new_node_mask = [diff(data.estimator.ekf.state.node_id) == 1 0] & trim;
for i = 1:3
 subplot(3,1,i)
 plot(data.estimator.ekf.state.time(trim), data.estimator.ekf.state.transform.translation(i,trim),'b.'); hold on;
...
for i = 1:3
 subplot(3,1,i)
 plot(data.estimator.ekf.state.time(trim), data.estimator.ekf.state.transform.euler(i,trim),'b.'); hold on;
...
for i = 1:3
 subplot(3,1,i)
 plot(data.estimator.ekf.state.time(trim), data.estimator.ekf.state.velocity(i,trim),'b.'); hold on;
...
for i = 1:3
 subplot(3,1,i)
 plot(data.estimator.ekf.snapshot.time(trim), data.estimator.ekf.snapshot.state(10+i,trim),'b.');
...
for i = 1:3
 subplot(3,1,i)
 plot(data.estimator.ekf.snapshot.time(trim), data.estimator.ekf.snapshot.state(13+i,trim),'b.');
 ylabel(strcat(labels{i},''));
 xlabel('time (sec)');
end
